...
# (c) 2016 neTear
""" 
nt_fp a optional service library to be included within service.ircClient
version 0.0.2 

			(nt_fp)this will do bot-command flood-protection on public channel and a bit more than that.
			You'll need to edit the main configuration fp_* to use it after inital start.
			At least set fp_floodprotection: true, otherwise the bot reacts
			and works like floodprotection.py is not in place.
			Once the bot started with fp_floodprotection:true any other fp_* are set
			to true by default, you might edit the configuration a second time for these.
			It's also up to you to have bot-command suggestions at all. fp_* should be self-explaining enough.
			Don't forget to stop the bot right before you edit the configureation!
			...
			So the plugin commands are adjustable in a very accurate manner
			The protection is involved even for !commands which are not part of the
			"internal kcs ~ known command set", so even a simple ! will be protected
			if fp_protect_any is true.
			...
			same bot-command._before_ they are being propcessed e.g. !figlet !tvtip and similar.
			So this will economising LOC within each plugin with similar functionalities
			and helps the user to find out your bot-commands as well.
			There is user_black_list (a sapce sperated string within config!), which drops
			bot-command-processing at all in the case of any given string-part (lowercase) is part 
			of the invoking nick!user@host. So bot-commands could be ignored for serveral
			users and or channels, e.g. other bots.
			
			To use this library, you simple need to put it into $bot/lib/
			...
			will stay in kcs until you delete them from floodprotection.config. You also may delete
...
class floodProtect:
	...
	#def internal_command(self,f_user,f_channel,f_command,f_options):
	#	#TODO query +authtenticated users only
	...
	def check(self, command, prefix, params):
		...
		if str(command)=='PRIVMSG':
			...
			try:
				...
				#if f_command == 'floodprotect':
				#	f_options = params[1].split(' ',1)[1:][0]
				#	floodProtect.internal_command(self,f_user,f_channel,f_command,f_options)
				#	return params, False
			except:
