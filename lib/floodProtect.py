...
version 0.0.3
            ...
            The plugin's 'known-commands-set' is adjustable in a very accurate manner.
...
"""
fp_known_command_dicts={
                        'data/ircClient.commands/commandsold.txt':['#otfbot'],\
                        'data/ircClient.wiki_commands/wiki_commands.txt':[]
                        }
"""
...
class floodProtect:
    class gFp():
        ...
        def checkChannelWhiteList(self, kcs, p_command, f_channel, whitelist_drops):
            ...
            try:
                ...
                return False
        def addCommand(self,g_command):
            print "adding "+g_command
    ...
    def addKnownCommand(self,g_command,fp_pt = None, fp_flags = None, fp_ch_wl = None):
        if g_command in self.kcs_:
            return g_command+" already in kcs_"
        fp_pt = fp_default_protect_time if fp_pt == None else fp_pt
        fp_flags = fp_default_flags if fp_flags == None else fp_flags
        fp_ch_wl = "" if fp_ch_wl == None else fp_ch_wl 
        self.kcs_.append(g_command)
        self.kcs_.sort()
        pluginname="tmp"
        self.kcs[g_command]=[fp_pt,\
                            fp_flags,\
                            fp_ch_wl,\
                            pluginname]
        return g_command+" added to kcs"
